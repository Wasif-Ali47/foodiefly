...
export default function DashBoardNavbar({ onItemClick }) {
  ...
  return (
    <>
      {/* Menu Button */}
      <button
        className="fixed top-4 left-4 z-10 bg-red-500 text-white p-3 rounded-lg md:hidden"
      ...
      >
        <div className="sidebar-header flex justify-between items-center p-4">
          <h5 className="w-full text-center text-xl bg-red-600 py-2 px-4 rounded-md text-white">
            Admin Menu
          </h5>

          {/* Close Button (Only for mobile) */}
          <button
            className="text-gray-800 bg-red-500 rounded p-2 mx-2 hover:bg-red-400 transition-colors md:hidden"
            onClick={handleToggle}
          >
        ...
        <ul className="list-unstyled p-4 space-y-3">
          {[
            ...
            <li
              ...
              className={`flex items-center mb-3 rounded-lg p-2 cursor-pointer transition-all duration-300 
                ${activeItem === item.name ? "bg-red-100 text-red-600" : "text-gray-800 hover:bg-red-50 hover:text-red-500"}`}
          ...
          <li
            className="flex items-center mb-3 cursor-pointer rounded-lg p-2 text-gray-800 hover:bg-red-50 hover:text-red-600 transition-all duration-300"
